...
const { createCompetencyAction, saveCompetencyRubricAction } = competencyActions;
...
const attackerCompetencyId = "77777777-7777-4777-8777-777777777777";
...
function rubricFormData() {
  const data = new FormData();
  data.set("organization_id", attackerOrganizationId);
  data.set("job_id", attackerJobId);
  data.set("competency_id", attackerCompetencyId);
  data.set("level_1", "  Cannot yet explain a job-relevant approach.  ");
  data.set("level_2", "Explains a partial approach with important gaps.");
  data.set("level_3", "Explains and applies a sound core approach.");
  data.set("level_4", "Applies the approach across relevant tradeoffs.");
  data.set("level_5", "Produces independently verifiable job-related evidence.");
  return data;
}
...
describe("competency server action", () => {
  ...
  it("exposes a dedicated route-bound rubric save action", () => {
    expect(saveCompetencyRubricAction).toBeTypeOf("function");
  });

  it("saves all five rubric levels through the authoritative route-bound RPC", async () => {
    ...
    ).resolves.toEqual({ status: "success", message: "Rubric saved." });

    expect(rpc).toHaveBeenCalledWith("save_competency_rubric", {
      p_organization_id: organizationId,
      p_job_id: jobId,
      p_competency_id: competencyId,
      p_level_1: "Cannot yet explain a job-relevant approach.",
      p_level_2: "Explains a partial approach with important gaps.",
      p_level_3: "Explains and applies a sound core approach.",
      p_level_4: "Applies the approach across relevant tradeoffs.",
      p_level_5: "Produces independently verifiable job-related evidence.",
    });
    expect(revalidatePath).toHaveBeenCalledWith(
      `/app/o/${organizationId}/jobs/${jobId}`,
    );
