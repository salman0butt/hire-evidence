...
|---|---|---|---|---|
| Repository/application foundation | Foundation sections / milestone ledger | Product Foundation | VERIFIED | PR #2 integrated to `main`; post-merge SHA `64ebeb4…` passed CI run `34486610200`. |
| Durable autonomous-development control plane | Owner framework + PRD autonomy sections | Product Foundation | VERIFIED | Integrated on `main`; framework/source/PRD gates pass on post-merge CI. |
...
| Reproducible dependency graph | Product Foundation build/CI requirements | Product Foundation | VERIFIED | `pnpm-lock.yaml` + frozen install pass on post-merge `main` CI. |
| M01.1 premium marketing shell + pricing + SEO | PRD 15–17, 196 | SaaS Shell + Auth | VERIFIED | `src/app/page.tsx`, typed pricing config, metadata/styles/tests/E2E; code head `6107253…` passed CI run `34492022676`. |
| Supabase SSR/session infrastructure | PRD 16, 196 | SaaS Shell + Auth | ACTIVE | Next legitimate slice M01.2; design/plan approved, implementation not started. |
| Core signup/login/logout/verification | PRD 16, 196 | SaaS Shell + Auth | PLANNED | M01.3 after SSR infrastructure. |
...
| Basic own-user profile + RLS | PRD 17, 196 | SaaS Shell + Auth | PLANNED | M01.6; cross-user isolation verification required. |
| Organizations and RBAC | PRD milestone roadmap | Organizations + RBAC | PLANNED | Requires complete M01; do not start early. |
