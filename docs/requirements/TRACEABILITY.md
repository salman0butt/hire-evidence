...
| AUTO-005 — Genuine RED → GREEN / root-cause debugging | Active capabilities | active milestone plans | test-first commits + CI logs | capability tests | settings RED #217 → GREEN #218; earlier M02 evidence retained | VERIFIED |
| AUTO-006 — Critical/Important findings block completion | Active capabilities | autonomy framework | review workflow | skeptical review | implemented M02.1–M02.7/isolation: 0 unresolved Critical/Important; final whole-milestone review pending | ACTIVE |
| AUTO-007 — Fresh exact-SHA CI | Active capabilities | autonomy framework | GitHub Actions | full quality job | isolation head `3e0c3555…` passed CI #219; documentation reconciliation requires fresh CI | ACTIVE |
...
| AUTO-009 — Durable recovery state | Active capabilities | autonomy framework | status/milestone/evidence docs | framework verifier | current M02 closeout state reconciled | ACTIVE |
...
| DEP-REPRO — Reproducible dependency graph | Product Foundation | foundation closeout | lockfile/frozen installs | CI install | CI #219 and later | VERIFIED |
...
| M02-A11Y-001 — Responsive/keyboard tenant closeout | Organizations + RBAC | M02 Task 8 | tenant/team/invitation/settings UI | desktop + 390×844 browser matrix | pending final closeout | ACTIVE |

## Active requirement interpretation

M00 and M01 are integrated and verified. M02 is the sole active product milestone in draft PR #4. Organization/membership foundation, fixed RBAC/input validation, onboarding, tenant shell, membership management, secure invitations, bounded settings, and real provider-backed Org A/Org B/unauthenticated isolation are verified slices. Milestone completion is not yet claimed because the plan-specific responsive/keyboard browser matrix, final whole-milestone review, final durable reconciliation and exact-final-head CI remain. PostgreSQL RLS/RPCs remain authoritative; AI has no authority in tenancy decisions and humans remain hiring decision makers.
